...
$TOOLS_HTML = [
  'add_to_cart' => <<<'tool_content_add_to_cart'
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8" />
...
<div class="container">
  ...
  <fieldset>
    ...
    <div class="hint">Multiple selection supportedâ€”each country is processed independently.</div>
  </fieldset>
...
623548"></textarea>
  ...
  <div id="loading">Processingâ€¦ please wait</div>

  <div class="output">
    <ul id="results"></ul>
    <p id="empty" style="display:none;color:#c00;font-weight:600">No results to show.</p>
    <div class="hint">
      <strong>Guest:</strong> <code>POST /api/v2/&lt;store&gt;/cart/createv2</code> â†’ use <code>data.result</code> as <code>quoteId</code> in <code>updateMultiple</code> (no token).<br>
      <strong>Logged-in:</strong> <code>POST /api/v2/&lt;store&gt;/user/login-v2</code> â†’ use <code>token</code> header <code>currenttoken</code> and <code>quote_id</code>.
    </div>
  </div>
</div>

<script>
/* ---------- Country â†’ store + code mapping ---------- */
const COUNTRY_MAP = {
...
function render(){
  ...
  rows.forEach(r=>{
    ...
    li.className = cls;
    li.innerHTML = `
      <strong>${r.country}</strong> â†’ Store: <code>${r.store}</code>
...
tool_content_add_to_cart,

  'asset_count' => <<<'tool_content_asset_count'
<!DOCTYPE html>
<html lang="en">
...
tool_content_asset_count,

  'brand' => <<<'tool_content_brand'
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8" />
...
<div class="container">
  ...
  <div class="panel">
    ...
    <div class="legend">
      <div class="row"><span class="status-badge ok">OK</span> <strong>OK</strong> â€“ Hits found (products returned).</div>
      <div class="row"><span class="status-badge warn">WARN</span> <strong>Warning</strong> â€“ No hits returned (empty results).</div>
      <div class="row"><span class="status-badge err">ERR</span> <strong>Error</strong> â€“ Fetch / parse problem (shown only in â€œShow Allâ€).</div>
    </div>
  </div>

  <footer>Â© 2024 Brand Link Checker</footer>
</div>
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€ Main Runner â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
async function run(){
  setLoading(true);
...
tool_content_brand,
  ...
  'category' => <<<'tool_content_category'
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8" />
...
<div class="container">
  ...
  <div class="panel">
    ...
    <div class="legend">
      <div class="row"><span class="status-badge ok">OK</span> <strong>OK</strong> â€“ Category returns products (hits > 0).</div>
      <div class="row"><span class="status-badge warn">WARN</span> <strong>Warning</strong> â€“ Category returns no products (empty hits).</div>
      <div class="row"><span class="status-badge warnnt">Wâ€‘NT</span> <strong>Warning NT</strong> â€“ *Warning* **with numeric category ID only** (subset of Warning).</div>
      <div class="row"><span class="status-badge err">ERR</span> <strong>Error</strong> â€“ Fetch / parse problem (shown only under â€œShow Allâ€).</div>
    </div>
  </div>

  <footer>Â© 2024 Category Link Checker</footer>
</div>
...
tool_content_category,

  'category_filter' => <<<'tool_content_category_filter'
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8"/>
...
https://www.jarir.com/api/v2/sa_en/cmspage/page-v2/235"></textarea>
  ...
  <footer>Â© 2024 Filtered-Category Link Checker</footer>

<script>
/* â”€â”€â”€â”€â”€ Cookie Helpers (kept for persistence) â”€â”€â”€â”€â”€ */
...
/* â”€â”€â”€â”€â”€ Process a parent URL â”€â”€â”€â”€â”€ */
...
/* Build URLs â€“ ONLY replace hyphen with comma for price or jb_discount_percentage values */
...
/* â”€â”€â”€â”€â”€ UI / Runtime Control â”€â”€â”€â”€â”€ */
...
tool_content_category_filter,

  'cms' => <<<'tool_content_cms'
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
<div class="container">
  ...
  <div class="panel">
    ...
    <div class="legend">
      <div class="row"><span class="status-badge ok">OK</span> <strong>OK</strong> â€“ JSON has <code>data === null</code> OR contains CMS items.</div>
      <div class="row"><span class="status-badge warn">WARN</span> <strong>Warning</strong> â€“ JSON fetched but no CMS items (empty).</div>
      <div class="row"><span class="status-badge err">ERR</span> <strong>Error</strong> â€“ Fetch / parse problem (only shown under â€œShow Allâ€).</div>
    </div>
  </div>

  <footer>Â© 2024 CMS Block Link Checker</footer>
</div>

<script>
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ State & Persistence â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ Crawl Logic â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
tool_content_cms,
  ...
  'getcategories' => <<<'tool_content_getcategories'
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8"/>
...
<body>
  ...
  <footer>Â© 2024 Get Categories Links</footer>
...
/* â”€â”€â”€â”€â”€ Process Parent â”€â”€â”€â”€â”€ */
async function processParent(parent){
  try{
    const res=await fetch(parent);
    if(!res.ok) throw new Error('Network response was not ok');
...
function splitBase(url){
  ...
  const basePrefix=parts.slice(0,5).join('/')+'/'; // â€¦/api/v2/
...
tool_content_getcategories,

  'headers_check' => <<<'tool_content_headers_check'
<!DOCTYPE html>
<html lang="en">
...
tool_content_headers_check,

  'images' => <<<'tool_content_images'
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8" />
...
https://www.jarir.com/api/v2/sa_en/cmspage/page-v2/235"></textarea>
  ...
  <footer>Â© 2024 Image Link Checker</footer>
...
/* â”€â”€â”€â”€â”€ Parent Processing â”€â”€â”€â”€â”€ */
async function processParent(parent){
  try{
    const res=await fetch(parent);
...
/* â”€â”€â”€â”€â”€ Extract Images (retain original logic / behavior) â”€â”€â”€â”€â”€ */
...
/* â”€â”€â”€â”€â”€ Check Images (kept functional behavior) â”€â”€â”€â”€â”€
...
/* â”€â”€â”€â”€â”€ Rendering & Filters â”€â”€â”€â”€â”€ */
function render(){
  const ul=document.getElementById('list');
  ul.innerHTML='';
  rows.forEach(r=>{
...
/* â”€â”€â”€â”€â”€ Copy Visible â”€â”€â”€â”€â”€ */
function copyVisible(){
  const links=[...document.querySelectorAll('#list li')]
    .filter(li=>li.style.display!=='none')
    .map(li=>li.querySelector('a').href);
...
/* â”€â”€â”€â”€â”€ Session / Runtime Reset â”€â”€â”€â”€â”€ */
function clearRuntime(){
  rows=[];
...
tool_content_images,
  ...
  'json_validator' => <<<'tool_content_json_validator'
<!DOCTYPE html>
<html lang="en">
    ...
    <script>
        async function run() {
            ...
            try {
                ...
                msg.textContent = 'âœ… Valid JSON';
                msg.className = 'status valid';
                msg.style.display = 'block';
                out.textContent = JSON.stringify(obj, null, 2);
                out.style.display = 'block';
            } catch (e) {
                msg.textContent = 'âŒ Invalid JSON: ' + e.message;
                msg.className = 'status invalid';
                msg.style.display = 'block';
            }
        }
...
tool_content_json_validator,

  'link_extractor' => <<<'tool_content_link_extractor'
<!DOCTYPE html>
<html lang="en">
...
tool_content_link_extractor,

  'login' => <<<'tool_content_login'
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8" />
...
user@example.com,SuperSecret!'></textarea>
  ...
  <div id="loading">Contacting login APIâ€¦</div>
...
<footer>Â© 2024 Login Flow Checker</footer>

<script>
/* Country â†’ store mapping */
const COUNTRY_MAP = {
...
function render(){
  ...
  attempts.forEach(a=>{
    ...
    li.innerHTML = `
      ...
      <div class="meta">${label}${a.message?` â€” ${escapeHtml(a.message)}`:''}</div>
    `;
    ul.appendChild(li);
  });
...
tool_content_login,
  ...
  'price_checker' => <<<'tool_content_price_checker'
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8" />
...
<div class="container">
  ...
  <p class="small">Paste one or more <strong>CMS page-v2</strong> parent URLs (e.g. <code>.../cmspage/page-v2/home</code>). We extract (banner image + CMS slug) â†’ OCR price â†’ open child CMS â†’ collect SKUs â†’ fetch <code>final_price</code> via <code>catalogv1/product/store/&lt;store&gt;/sku/&lt;SKUS&gt;</code> â†’ compare.</p>
  ...
  <div class="panel">
    ...
    <div class="row">
      ...
      <input id="hfToken" type="password" placeholder="(optional) hf_â€¦ token" style="min-width:260px">
      <span class="small">Serve via http(s) to use HF; browsers block it from <code>file://</code> (CORS).</span>
    </div>
  </div>

  <div id="loading" class="loading">Processingâ€¦ (OCR + API fetch)</div>
  ...
  <div class="panel">
    <h3>Paired Results (Banner â†” Comparison)</h3>
  ...
  <div class="panel">
    ...
    <p class="small">One per line: <code>imageURL || cmsSlugOrSkuUrl</code>. A slug like <code>apple-iphone-17</code> is resolved using the first parentâ€™s store; a full product API URL is used directly.</p>
  ...
  <footer style="margin-top:14px;text-align:center;color:#888;font-size:13px">Â© 2025 Banner Price Checker</footer>
</div>
...
function extractPriceCandidatesFromText(t){
  t = toWesternDigits(t).replace(/(?:sar|aed|kwd|qar|bhd?|rs|riyals?|ï·¼|Ø±\.?Ø³\.?)/gi,' ')
...
async function preprocessToDataURL(blob, {enhance=true}={}){
  ...
  // guarantee workable width 1200â€“2000 px
...
async function tesseractPass(img, psm){
  return await Tesseract.recognize(
    img, 'eng+ara',
    {
      ...
      tessedit_char_whitelist: "0123456789,.Ù Ù¡Ù¢Ù£Ù¤Ù¥Ù¦Ù§Ù¨Ù©Û°Û±Û²Û³Û´ÛµÛ¶Û·Û¸Û¹",
...
async function connectSpace(){
  if(!GradioClient) throw new Error('HF disabled on file:// â€” serve via http(s)');
...
async function ocrImage(url, enhance){
  ...
  if(requested==='hf' && location.protocol!=='file:'){
    try{ return await runSpaceOCROnUrl(url); }catch(e){ console.warn('HF failed â†’ Tesseract:', e); }
  }
  return ocrWithTesseractPro(url, enhance);
}

/* ============ CMS â†’ pairs / SKUs / prices ============ */
...
function appendDiscovery(parentUrl, pairs){
  ...
  <div class="pair" style="grid-template-columns:1fr">
    <div class="left" style="border-right:none">
      ...
      <div style="margin-top:6px">${pairs.slice(0,50).map(p=>`<span class="pill">${p.cmsSlug||'<em>unknown</em>'}</span>`).join(' ')}${pairs.length>50?' â€¦':''}</div>
    </div>
  </div>`;
...
tool_content_price_checker,

  'products' => <<<'tool_content_products'
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8" />
...
https://www.jarir.com/api/v2/sa_en/cmspage/page-v2/235"></textarea>
    ...
    <div class="output">
      ...
      <div class="legend">
        ...
        <div class="legend-row"><span class="badge warnplus">Warning Plus</span> No matching SKUs returned (also shown under â€œWarningâ€).</div>
        <div class="legend-row"><span class="badge err">Error</span> Request / parse problem.</div>
      </div>
    </div>
  </div>

  <footer>Â© 2024 Product Link Checker</footer>

<script>
/* â”€â”€â”€â”€â”€ Cookie Helpers (future-proof; currently only used for clearSession completeness) â”€â”€â”€â”€â”€ */
function setCookie(n,v,d){const e=new Date(Date.now()+d*864e5).toUTCString();document.cookie=`${n}=${encodeURIComponent(v)};expires=${e};path=/`;}
function getCookie(n){return document.cookie.split('; ').reduce((r,v)=>{const p=v.split('=');return p[0]===n?decodeURIComponent(p[1]):r},'');}
function deleteCookie(n){setCookie(n,'',-1);}
...
/* â”€â”€â”€â”€â”€ Core Run â”€â”€â”€â”€â”€ */
async function run(){
  // We KEEP previous results if user runs again to append? Requirement: re-run fresh each fetch.
  // For consistent â€œsessionâ€ model we clear runtime each run (except textarea).
  clearRuntime(); 
  toggleLoading(true);
...
/* â”€â”€â”€â”€â”€ Rendering & Filtering â”€â”€â”€â”€â”€ */
function render(){
  const ul=document.getElementById('list');
  ul.innerHTML='';
  rows.forEach(r=>{
...
/* â”€â”€â”€â”€â”€ Session Clearing â”€â”€â”€â”€â”€ */
function clearRuntime(){
  rows=[];
  visitedParents.clear();
  visitedLinks.clear();
...
tool_content_products,

  'sku' => <<<'tool_content_sku'
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
<div class="container">
  ...
  <div class="panel">
    ...
    <div class="legend">
      <div class="row"><span class="status-badge ok">OK</span> <strong>OK</strong> â€“ Hits array contains one or more products.</div>
      <div class="row"><span class="status-badge warn">WARN</span> <strong>Warning</strong> â€“ Hits array empty for that SKU.</div>
      <div class="row"><span class="status-badge err">ERR</span> <strong>Error</strong> â€“ Fetch / parse issue (shown only in *Show All* or *Error* filter).</div>
    </div>
  </div>

  <footer>Â© 2024 Single SKU Link Checker</footer>
</div>
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ Core Flow â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
async function run(){
...
tool_content_sku,
  ...
  'speed_test' => <<<'tool_content_speed_test'
<!DOCTYPE html>
<html lang="en">
...
tool_content_speed_test,

  'stock' => <<<'tool_content_stock'
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8" />
...
https://www.jarir.com/api/v2/ae_en/cmspage/page-v2/12"></textarea>
  ...
  <footer>Â© 2024 Stock / Availability Checker</footer>
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€ Clear Search â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€ Main Run â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
async function run(){
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€ Parent Processing â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
async function processParent(parent){
  try{
    const res = await fetch(parent);
    if(!res.ok) throw new Error('Network error');
...
/* Extract SKUs from various CMS patterns:
   - product || <sku>
   - products || <csv skus>
   - new_collection â€¦ || category || <catId> || <csv skus>
*/
function extractSkus(json){
  const items = json?.data?.cms_items?.items || [];
  const found = [];
  ...
  items.forEach(({item})=>{
    ...
    // 2) new_collection â€¦ || category || <catId> || <csv skus>
    for(let i=0;i<tokens.length;i++){
...
/* â”€â”€â”€â”€â”€â”€â”€â”€â”€ Rendering & Filters â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
function toggleLoading(on){
  document.getElementById('loading').style.display = on ? 'block':'none';
}
...
tool_content_stock,
  ...
  'sub_category' => <<<'tool_content_sub_category'
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8"/>
...
<body>
  ...
  <footer>Â© 2024 Sub-Category Link Checker</footer>
...
<body>
  ...
  <div id="modal-report" class="modal-overlay">
    <div class="modal-content"
      style="max-width: 1000px; width:95%; height:90vh; padding:0; overflow:hidden; display:flex; flex-direction:column; background:#fff;">
      <div style="display:flex; justify-content:flex-end; padding:8px; background:#1a3a57;">
      ...
      <iframe id="report-iframe" style="flex:1; width:100%; border:none;"></iframe>
    </div>
  </div>
  ...
  <script>
    ...
    window.triggerOpenIssuesReport = function () {
      ...
      // Get current filters
      const uidVal = document.getElementById('filter-user') ? document.getElementById('filter-user').value : '';

      let params = new URLSearchParams();
      if (uidVal) params.append('user_id', uidVal);
      params.append('status', 'failed'); // Open Issues = Failed/Warning

      const url = 'qa_run_report.php?' + params.toString();
      openReportModal(url);
    };
