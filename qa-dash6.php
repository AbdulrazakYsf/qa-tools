...
.log-form{display:flex;flex-wrap:wrap;gap:8px;align-items:flex-end;margin-bottom:10px;}
.log-form .form-field{min-width:120px;}
  .run-console {
    background: #0d1117;
    color: #c9d1d9;
  ...
  .run-console .log-error { color: #ff7b72; }
  .run-console .log-success { color: #7ee787; }
  ...
  <section id="tab-dashboard" class="tab-content active">
    ...
    <div class="dashboard-grid">
      <div class="section-card">
        ...
        <div class="text-muted" style="margin-top:8px;">
          Modules are the embedded tools from your original HTML files.
        </div>
      </div>
      ...
      <div class="section-card">
        <div class="tool-runner">
            ...
            <div>
              <strong id="active-tool-name">No tool selected</strong>
              <div class="text-muted" id="active-tool-config-info">Choose a module on the left.</div>
            </div>
            <div>
              <button class="btn-ghost" id="btn-run-all">Run All Tests</button>
            </div>
          </div>
          
          <div id="dash-console" class="run-console" style="display:none;"></div>

          <div class="tool-container">
            <iframe id="tool-iframe" class="tool-iframe"></iframe>
          </div>
        </div>
      </div>
    </div>
...
async function selectModule(code){
  ACTIVE_TOOL = code;
  document.querySelectorAll('.module-tile').forEach(m=>{
    m.classList.toggle('active', m.dataset.code === code);
  });
  ...
  document.getElementById('active-tool-config-info').textContent =
    cfg ? `Using config: ${cfg.config_name}` : 'No saved config for this tool.';
  loadToolIntoIframe(code);
}
...
document.getElementById('btn-run-all').addEventListener('click', async ()=>{
  ...
  // Clear Console
  const con = document.getElementById('dash-console');
  con.innerHTML = '';
  ...
  for (const code of selectedCodes) {
    ...
    if (!cfg || !inputsValid) {
       ...
       totalFailed++; // Count as failed tool run
       totalOpen++;
       continue;
    }
    ...
    try {
      ...
      logToConsole(`${code} Finished. Pass: ${result.passed}, Fail: ${result.failed}`, result.failed>0?'error':'success');
    } catch (e) {
  ...
  try {
    ...
    alert('Run All Tests completed.\nTotal: ' + totalTests + '\nPassed: ' + totalPassed + '\nFailed: ' + totalFailed + '\nOpen issues: ' + totalOpen);
  } catch (e) {
    console.error(e);
    logToConsole('Error Saving Run: ' + e.message, 'error');
    alert('Run All Tests completed but logging failed: ' + e.message);
  } finally {
    btn.disabled = false;
  }
});
