...
document.getElementById('btn-run-all').addEventListener('click', async ()=>{
  ...
  // Build execution plan: selected tools must have enabled config
  const missing = [];
  const plan = [];
  selectedCodes.forEach(code=>{
    const cfg = CONFIGS.find(c=>c.tool_code===code && c.is_enabled==1);
    if (!cfg) missing.push(code);
    else plan.push({code, cfg});
  });

  if (missing.length) {
    alert('Missing enabled configuration for: ' + missing.join(', ') + '.\nPlease add and enable configurations first.');
    return;
  }
  ...
  for (const item of plan) {
    try {
      ...
      console.error('Error running tool', item.code, e);
      totalFailed += 1;
      totalOpen   += 1;
    }
  }
