...
function qa_db(): PDO {
    ...
    try {
        $pdo->exec("ALTER TABLE qa_users ADD COLUMN password_hash VARCHAR(255) NULL");
    } catch(PDOException $e) {}
    try {
        $pdo->exec("ALTER TABLE qa_users ADD COLUMN verification_token VARCHAR(100) NULL");
    } catch(PDOException $e) {}
    try {
        $pdo->exec("ALTER TABLE qa_users ADD COLUMN token_expires_at DATETIME NULL");
    } catch(PDOException $e) {}
        ...
        switch ($action) {
            ...
            case 'save-user':
                ...
                if ($id) {
                    ...
                    // Insert with token
                    $stmt = $db->prepare("INSERT INTO qa_users (name, email, role, is_active, verification_token, token_expires_at) VALUES (?,?,?,?,?,?)");
                    $stmt->execute([$name, $email, $role, $is_active, $token, $expires]);
                    $id = $db->lastInsertId();

                    // Send Email
                    $verifyLink = "http://" . $_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . "/verify.php?token=$token";
                    $subject = "Welcome to QA Dashboard - Verify Email";
                    $msg = "Hello $name,\n\nAn account has been created for you on the QA Dashboard.\nPlease click the link below to set your password:\n\n$verifyLink\n\nThis link expires in 48 hours.";
                    $headers = "From: no-reply@qa-dashboard.com";
...
<div class="app-shell">
  <header class="app-header">
    ...
    <div style="text-align:right">
        <small>Logged in as: <b><?php echo htmlspecialchars($currentUser['name']); ?></b> (<?php echo $currentUser['role']; ?>)</small><br>
        <a href="logout.php" style="font-size:12px;color:#d32f2f;">Logout</a>
    </div>
  </header>

  <div class="tabs">
    <button class="tab-btn active" data-tab="dashboard">Dashboard</button>
    <?php if(in_array($currentUser['role'], ['admin', 'tester'])): ?>
